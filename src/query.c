...
#define Trace(p1, p2, p3, p4)                                                                      \
    if (q->trace /*&& !consulting*/)                                                               \
    trace_call(p1, p2, p3, p4)
...
void check_pressure(query *q)
{
    ...
    if (q->trails_size > (INITIAL_NBR_TRAILS * PRESSURE_FACTOR)) {
        ...
        printf("*** q->st.tp=%u, q->trails_size=%u\n", (unsigned)q->st.tp,
               (unsigned)q->trails_size);
#endif
        q->trails_size =
            alloc_grow(q, (void **)&q->trails, sizeof(trail), q->st.tp, q->st.tp * 5 / 4);
    }
...
#if TRACE_MEM
        printf("*** q->st.new_fp=%u, q->frames_size=%u\n", (unsigned)q->st.new_fp,
               (unsigned)q->frames_size);
#endif
        q->frames_size =
            alloc_grow(q, (void **)&q->frames, sizeof(frame), q->st.new_fp, q->st.new_fp * 5 / 4);
    }
...
static bool check_choice(query *q)
{
    ...
    q->realloc_choices++;
    pl_idx new_choicessize =
        alloc_grow(q, (void **)&q->choices, sizeof(choice), q->cp, q->choices_size * 5 / 4);
...
bool check_frame(query *q, unsigned max_vars)
{
    ...
    q->realloc_frames++;
    pl_idx new_framessize =
        alloc_grow(q, (void **)&q->frames, sizeof(frame), q->st.new_fp, q->frames_size * 5 / 4);
...
bool check_trail(query *q)
{
    ...
    q->realloc_trails++;
    pl_idx new_trailssize =
        alloc_grow(q, (void **)&q->trails, sizeof(trail), q->st.tp, q->trails_size * 5 / 4);
...
static size_t scan_is_chars_list_internal(query *q, cell *l, pl_ctx l_ctx, bool allow_codes,
                                          bool *has_var, bool *is_partial, cell **cptr)
{
...
static void push_frame(query *q)
{
    ...
    if (q->pl->opt && is_end(next_cell) && !next_cell->ret_instr && (fold->prev != CTX_NUL)) {
        fnew->prev = fold->prev;
...
static void commit_frame(query *q)
{
    ...
    if (!q->no_recov && last_match && (q->st.new_fp == (q->st.cur_ctx + f->frame_size)) // Top frame
    ) {
...
bool push_choice(query *q)
{
    ...
    ch->catchme_retry = ch->catchme_exception = ch->barrier = ch->register_cleanup =
        ch->block_catcher = ch->fail_on_retry = ch->succeed_on_retry = ch->reset = false;
...
static bool resume_frame(query *q)
{
    ...
    if (q->pl->opt && !f->no_recov && (q->st.new_fp == (q->st.cur_ctx + f->frame_size)) // Top frame
...
bool has_next_key(query *q)
{
    ...
    if (q->st.dbe->cl.is_unique) {
        ...
        if ((q->st.key->arity == 3) && q->st.karg1_is_atomic && q->st.karg2_is_atomic &&
            q->st.karg3_is_atomic)
            return false;
    }
...
static bool expand_meta_predicate(query *q, predicate *pr)
{
    ...
    for (cell *k = q->st.key + 1, *m = pr->meta_args + 1; arity--;
        ...
        if (!pr) {
            ...
            if (get_builtin_term(q->st.m, p1, &found, NULL), found) {
                if (is_retract != DO_CLAUSE)
                    return throw_error(q, p1, p1_ctx, "permission_error",
