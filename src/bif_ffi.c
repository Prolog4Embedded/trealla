...
static void register_struct(prolog *pl, const char *name, unsigned arity, void *fn, uint8_t *types,
                            const char **names)
{
...
static void register_ffi(prolog *pl, const char *name, unsigned arity, void *fn, uint8_t *types,
                         uint8_t ret_type, const char *ret_name, bool evaluable)
{
...
bool bif_sys_register_predicate_4(query *q)
{
    ...
    return do_register_predicate(q->st.m, q, (void *)(size_t)get_smallint(p1), C_STR(q, p2), p3,
                                 p3_ctx, C_STR(q, p4));
}
...
static bool handle_struct1(query *q, foreign_struct *sptr, nested_elements *nested, ffi_type *types,
                           unsigned *pdepth)
{
...
static void handle_struct2(query *q, nested_elements *nested, unsigned *pdepth, unsigned cnt,
                           uint8_t *bytes, size_t *boff, cell *h, pl_ctx h_ctx, void **arg_values,
                           unsigned *p_pos)
{
    ...
    if (nested[depth].elements[cnt - 1] == &ffi_type_uint8) {
        ...
        while (is_iso_list(l)) {
            ...
            if (cnt > 0) {
                handle_struct2(q, nested, pdepth, cnt, bytes, &bytes_offset, h, h_ctx, arg_values,
                               &pos);
            }
...
bool wrap_ffi_predicate(query *q, builtins *ptr)
{
    ...
    for (unsigned i = 0; i < arity; i++) {
        ...
        if (ptr->types[i] == FFI_TAG_UINT8) {
            ...
            while (is_iso_list(l)) {
                ...
                if (cnt > 0) {
                    handle_struct2(q, nested, &pdepth, cnt, bytes, &bytes_offset, h, h_ctx,
                                   arg_values, &pos);
                }
    ...
    if (true /* !ptr->ffi_ret_type */) {
        ...
        case (FFI_TAG_STRUCT): {
            ...
            // printf("wrapper: arity=%u, found struct return type: %s, arity=%u, depth=%u,
            // pdepth=%u\n", arity, name, sptr->arity, depth, pdepth); unsigned save_depth =
            // ++pdepth;
...
builtins g_ffi_bifs[MAX_FFI] = {
    ...
    {"$register_function", 4, bif_sys_register_function_4, "+term,+atom,+list,+atom", false, false,
     BLAH},
    {"$register_predicate", 4, bif_sys_register_predicate_4, "+term,+atom,+list,+atom", false,
     false, BLAH},
