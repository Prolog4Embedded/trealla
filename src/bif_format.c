    ...
    while (is_more_data(q, &fmt1)) {
        ...
        case 'E': {
            ...
            if (argval || argval_specified) {
                if (ch == 'e')
                    // len = snprintf(dst, len, "%.*e", argval?argval:1, is_float(c) ?
                    // (argval?get_float(c):floor(get_float(c))) : get_smallint(c));
                    len = snprintf(dst, len, "%.*e", argval,
                                   is_float(c) ? get_float(c) : get_smallint(c));
                else
                    // len = snprintf(dst, len, "%.*", argval?argval:1, is_float(c) ?
                    // (argval?get_float(c):floor(get_float(c))) : get_smallint(c));
                    len = snprintf(dst, len, "%.*E", argval,
                                   is_float(c) ? get_float(c) : get_smallint(c));
            } else {
        ...
        case 'G': {
            ...
            if (argval || argval_specified) {
                if (ch == 'g')
                    // len = snprintf(dst, len, "%.*g", argval?argval:1, is_float(c) ?
                    // (argval?get_float(c):floor(get_float(c))) : get_smallint(c));
                    len = snprintf(dst, len, "%.*g", argval,
                                   is_float(c) ? get_float(c) : get_smallint(c));
                else
                    // len = snprintf(dst, len, "%.*G", argval?argval:1, is_float(c) ?
                    // (argval?get_float(c):floor(get_float(c))) : get_smallint(c));
        ...
        case 'f': {
            ...
            if (argval || argval_specified)
                // len = snprintf(dst, len, "%.*f", argval?argval:1, is_float(c) ?
                // (argval?get_float(c):floor(get_float(c))) : get_smallint(c));
                len = snprintf(dst, len, "%.*f", argval,
                               is_float(c) ? get_float(c) : get_smallint(c));
            else
    ...
    if (str == NULL) {
        ...
        while (len) {
            ...
            tmpsrc += tmpbuf_free;
               ...
               ((CMP_STRING_TO_CSTR(q, str, "atom") && CMP_STRING_TO_CSTR(q, str, "chars") &&
