...
static const char *pl4bm_format_bytes(size_t bytes, double *out_value);
...
{
    if (bytes >= (1ull << 30)) {
        *out_value = (double)bytes / (double)(1ull << 30);
        return "GiB";
    }
    if (bytes >= (1ull << 20)) {
        *out_value = (double)bytes / (double)(1ull << 20);
        return "MiB";
    }
    if (bytes >= (1ull << 10)) {
        *out_value = (double)bytes / (double)(1ull << 10);
        return "KiB";
    }

    *out_value = (double)bytes;
    return "B";
}
