...
cell *list_head(cell *l, cell *tmp)
{
    ...
    const char *src = is_slice(l)    ? l->val_str
                      : is_strbuf(l) ? (char *)l->val_strb->cstr + l->strb_off
                                     : (char *)l->val_chr + l->val_off;
    tmp->num_cells = 1;
    tmp->flags = 0;
    tmp->arity = 0;
...
cell *list_tail(cell *l, cell *tmp)
{
    ...
    const char *src = is_slice(l)    ? l->val_str
                      : is_strbuf(l) ? (char *)l->val_strb->cstr + l->strb_off
                                     : (char *)l->val_chr;
    size_t char_len = len_char_utf8(src);
    size_t str_len = is_slice(l)    ? (size_t)l->str_len
                     : is_strbuf(l) ? (size_t)l->val_strb->len - l->strb_off
                                    : (unsigned)l->chr_len;
...
cell *make_nil(void)
{
    static cell tmp = {.tag = TAG_INTERNED, .num_cells = 1, .flags = 0, .arity = 0, .val_off = 0};
...
static bool conditionals(parser *p, cell *d)
{
    ...
    if (!strcmp(dirname, "elif") && (c->arity == 1) && !p->m->ifs_done[p->m->if_depth] &&
        p->m->ifs_blocked[p->m->if_depth]) {
        bool ok = goal_run(p, FIRST_ARG(c));
        p->m->ifs_blocked[p->m->if_depth] = !ok;
        p->m->ifs_done[p->m->if_depth] = ok;
        return true;
    ...
    if (!strcmp(dirname, "else") && (c->arity == 0) && !p->m->ifs_done[p->m->if_depth] &&
        p->m->ifs_blocked[p->m->if_depth]) {
...
static bool directives(parser *p, cell *d)
{
    ...
    if (!strcmp(dirname, "module") && (c->arity >= 1)) {
        ...
        while (is_iso_list(p2)) {
            ...
            if (is_compound(head)) {
                if (!strcmp(C_STR(p, head), "/") || !strcmp(C_STR(p, head), "//")) {
                    cell *f = head + 1, *a = f + 1;
                    if (!is_interned(f))
                        return true;
                    if (!is_integer(a))
                    ...
                    if (!pr)
                        pr = create_predicate(p->m, &tmp, NULL);
    ...
    if ((!strcmp(dirname, "use_module") || !strcmp(dirname, "autoload") ||
         !strcmp(dirname, "reexport")) &&
        (c->arity >= 1)) {
        if (!is_callable(p1))
            return true;
    ...
    if (is_iso_list(p1)) {
        ...
        while (is_list(p1)) {
            ...
            if (is_interned(h) && (!strcmp(C_STR(p, h), "/") || !strcmp(C_STR(p, h), "//")) &&
                (h->arity == 2)) {
                cell *c_name = h + 1;
                ...
                if (!strcmp(dirname, "dynamic")) {
                    ...
                    if (pr && !pr->is_dynamic && pr->head) {
                        if (!p->do_read_term)
                            fprintf(
                                stderr,
                                "Error: no permission to modify static predicate %s:%s/%u, %s:%d\n",
    ...
    while (is_interned(p1) && (p1->val_off != g_dot_s)) {
        ...
        if ((!strcmp(C_STR(p, c_id), "/") || !strcmp(C_STR(p, c_id), "//")) && (p1->arity == 2)) {
            cell *c_name = c_id + 1;
...
void assign_vars(parser *p, unsigned start, bool rebase)
{
    ...
    for (unsigned i = 0; i < cl->num_vars; i++) {
        if (p->is_consulting && !p->do_read_term &&
            (p->vartab.used[i] == 1)
            // && (p->vartab.name[i][strlen(p->vartab.name[i])-1] != '_')
            && (GET_POOL(p, p->vartab.off[i])[0] != '_')) {
            if (!p->pl->quiet && !((cl->cells->val_off == g_neck_s) && cl->cells->arity == 1))
                fprintf(stderr, "Warning: singleton: %s, near %s:%d\n",
                        GET_POOL(p, p->vartab.off[i]), get_loaded(p->m, p->m->filename),
                        p->line_num);
        }
    }
...
static bool reduce(parser *p, pl_idx start_idx, bool last_op)
{
    ...
    for (pl_idx i = start_idx; i <= end_idx;) {
        ...
        if (is_fx(c)) {
            ...
            if ((((pl_idx)(rhs - p->cl->cells)) < end_idx) && is_xf(rhs) &&
                (rhs->priority == c->priority)) {
                if (!p->do_read_term)
                    fprintf(stderr, "Error: syntax error, operator clash, %s:%d\n",
                            get_loaded(p->m, p->m->filename), p->line_num);
        ...
        *c = save;
        ...
        if (is_xfx(c)) {
            ...
            if ((i <= end_idx) && (is_xfx(next)) && (next->priority == c->priority)) {
                if (!p->do_read_term)
                    fprintf(stderr, "Error: syntax error, operator clash, %s:%d\n",
                            get_loaded(p->m, p->m->filename), p->line_num);
...
static void expand_meta_predicate(parser *p, predicate *pr, cell *goal)
{
    ...
    for (cell *k = goal + 1, *m = pr->meta_args + 1; arity--;
         k += k->num_cells, m += m->num_cells) {
        cell tmpbuf[2];
...
PARSE_LOOP:
    ...
    if ((*s == '0') && (s[1] == 'b')) {
        ...
        if (mp_int_to_int(&v2, &val) == MP_RANGE) {
            ...
            if (neg)
                p->v.val_bigint->ival.sign = MP_NEG;
...
TRY_AGAIN:
    ...
    if ((*src == '"') || (*src == '`') || (*src == '\'')) {
        ...
        for (;;) {
            ...
            for (; *src && (ch = get_char_utf8(&src));) {
                ...
                if ((ch == '\\') && p->flags.character_escapes) {
                    ...
                    if (!p->error) {
                        ...
                        if (!p->do_read_term)
                            fprintf(stderr,
                                    "Error: syntax error, illegal character escape <<%s>>, %s:%d\n",
            ...
            if (!p->is_string && SB_strcmp(p->token, "[") && SB_strcmp(p->token, "(") &&
    ...
    if (is_matching_pair(ch, next_ch, ')', '(') || is_matching_pair(ch, next_ch, ']', '(') ||
        is_matching_pair(ch, next_ch, '}', '(') || is_matching_pair(ch, next_ch, '}', '(')) {
        if (!p->do_read_term)
...
unsigned tokenize(parser *p, bool is_arg_processing, bool is_consing)
{
    ...
    while (get_token(p, last_op, last_postfix)) {
        ...
        if (!p->quote_char && !SB_strcmp(p->token, ".") && (*p->srcptr != ',') &&
            (*p->srcptr != '(') && (*p->srcptr != ')') && (*p->srcptr != ']') &&
            (*p->srcptr != '}') && !iswalnum(*p->srcptr) && (*p->srcptr != '_') &&
            ((*p->srcptr != ' ') || !p->is_op)) {
            ...
            if (analyze(p, 0, last_op)) {
                if (p->cl->cells->num_cells < p->cl->cidx) {
                    if (!p->do_read_term)
                        printf(
                            "Error: syntax error, operator expected unfinished input '%s', %s:%d\n",
                ...
                if ((p->is_consulting /*|| p->is_command*/) && !p->skip &&
                    check_body_callable(p->cl->cells)) {
                    if (!p->do_read_term)
        ...
        if (!p->quote_char && !is_arg_processing && !is_consing && last_op && !last_postfix &&
            !SB_strcmp(p->token, ",")) {
            if (!p->do_read_term)
        ...
        if (!p->quote_char && is_arg_processing && !is_consing && p->is_op /*&& last_op*/
            ...
            if (!last_op && (priority > 999)) {
                if (!p->do_read_term)
                    fprintf(stderr,
                            "Error: syntax error, parens needed around operator '%s', %s:%d\n",
                            SB_cstr(p->token), get_loaded(p->m, p->m->filename), p->line_num);
            ...
            if ((arg_idx == p->cl->cidx) || !p->cl->cidx) {
                if (!p->do_read_term)
                    fprintf(stderr, "Error: syntax error, missing arg '%s', %s:%d\n",
                            p->save_line ? p->save_line : "", get_loaded(p->m, p->m->filename),
                            p->line_num);
        ...
        if (!p->quote_char && ((is_arg_processing && !SB_strcmp(p->token, ",")) ||
        ...
        if (!p->is_quoted /*&& is_consing*/ && (p->start_term || last_op) &&
            !SB_strcmp(p->token, "|")) {
            if (!p->do_read_term)
        ...
        if (is_interned(&p->v)) {
            ...
            int nextch = *s;
            bool noneg =
                (!SB_strcmp(p->token, "-") || !SB_strcmp(p->token, "+")) && (nextch == '='); // Hack
        ...
        if (!SB_strcmp(p->token, "!") && ((*p->srcptr == ')') || (*p->srcptr == ';') ||
                                          (*p->srcptr == ',') || (*p->srcptr == '.')))
            p->quote_char = 1;
        ...
        if (priority && (last_op || last_bar) && !IS_POSTFIX(specifier)) {
            char *s = eat_space(p);
        ...
        if (!p->is_op && !is_func && last_op && last_postfix && 0) {
            if (!p->do_read_term)
                fprintf(stderr,
                        "Error: syntax error, near '%s', operator expected postfix '%s', %s:%d\n",
                        SB_cstr(p->token), p->save_line ? p->save_line : "",
                        get_loaded(p->m, p->m->filename), p->line_num);
        ...
        if (is_bigint(&p->v)) {
            ...
            feclearexcept(FE_INVALID | FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW);
                   ...
                   (!p->is_quoted || is_func || p->is_op || p->is_var || p->is_consulting ||
                    (get_builtin(p->pl, SB_cstr(p->token), SB_strlen(p->token), 0, &found, NULL),
                     found) ||
                    !SB_strcmp(p->token, "[]"))) {
            ...
            if (p->is_var) {
                c->tag = TAG_VAR;
                c->var_num =
                    (2 * p->nesting_braces) + (2 * p->nesting_brackets) + p->nesting_parens;
            }
