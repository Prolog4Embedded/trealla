...
#define is_boolean(c)                                                                              \
    ((is_interned(c) && !(c)->arity && (((c)->val_off == g_true_s) || ((c)->val_off == g_false_s))))
...
#define is_zero(c)                                                                                 \
    (is_bigint(c)     ? mp_int_compare_zero(&(c)->val_bigint->ival) == 0                           \
     : is_smallint(c) ? get_smallint(c) == 0                                                       \
     : is_float(c)    ? get_float(c) == 0.0                                                        \
                      : false)

#define is_negative(c)                                                                             \
    (is_bigint(c)     ? (c)->val_bigint->ival.sign == MP_NEG                                       \
     : is_smallint(c) ? get_smallint(c) < 0                                                        \
     : is_float(c)    ? get_float(c) < 0.0                                                         \
                      : false)

#define is_positive(c)                                                                             \
    (is_bigint(c)     ? mp_int_compare_zero(&(c)->val_bigint->ival) > 0                            \
     : is_smallint(c) ? get_smallint(c) > 0                                                        \
     : is_float(c)    ? get_float(c) > 0.0                                                         \
                      : false)

#define is_not_less_than_zero(c)                                                                   \
    (is_bigint(c)     ? mp_int_compare_zero(&(c)->val_bigint->ival) >= 0                           \
     : is_smallint(c) ? get_smallint(c) >= 0                                                       \
     : is_float(c)    ? get_float(c) >= 0.0                                                        \
                      : false)
...
#define _CSTRING_LEN(c) (is_strbuf(c) ? (c)->strb_len : is_slice(c) ? (c)->str_len : (c)->chr_len)

#define _C_STR(pl, c) (!is_cstring(c) ? (g_global_atoms + (c)->val_off) : _CSTRING_STR(c))

#define _C_STRLEN(pl, c) (!is_cstring(c) ? strlen(g_global_atoms + (c)->val_off) : _CSTRING_LEN(c))
...
#define _CMP_SLICES(pl, c1, c2)                                                                    \
    slicecmp(_C_STR(pl, c1), _C_STRLEN(pl, c1), _C_STR(pl, c2), _C_STRLEN(pl, c2))
...
struct run_state_ {
    ...
    union {
        struct {
            cell *key;
            pl_ctx key_ctx;
            bool karg1_is_ground : 1, karg2_is_ground : 1, karg3_is_ground : 1, karg1_is_atomic : 1,
                karg2_is_atomic : 1, karg3_is_atomic : 1;
        };
        struct {
            uint64_t uv1, uv2;
        };
...
struct prolog_ {
    ...
    struct {
        pl_idx tab1[MAX_IGNORES], tab2[MAX_IGNORES];
    };
    list modules;
