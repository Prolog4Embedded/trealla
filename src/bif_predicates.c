...
static pl_idx queue_used(const query *q)
{
    return q->qp[0];
}
static cell *get_queue(query *q)
{
    return q->queue[0];
}
...
static bool bif_iso_notunifiable_2(query *q)
{
    ...
    SET_OP(&tmp2, OP_XFX);
    cell *tmp =
        prepare_call(q, CALL_NOSKIP, &tmp2, q->st.cur_ctx, p1->num_cells + p2->num_cells + 4);
    pl_idx num_cells = 0;
...
static bool bif_iso_number_chars_2(query *q)
{
    ...
    if (!is_var(p2) && !any_vars) {
        ...
        if (!is_number(&p->v) || p->error || p->nesting_parens || p->nesting_braces ||
            p->nesting_brackets || (p->cl->cidx > 1)) {
            SB_free(pr);
            bool ok = throw_error(q, orig_p2, p2_ctx, "syntax_error",
                                  p->error && p->error_desc ? p->error_desc : "unexpected_char");
            return ok;
        }
...
static bool bif_iso_atom_codes_2(query *q)
{
    ...
    if (!is_var(p2) && is_var(p1)) {
        ...
        while (is_list(p2)) {
            ...
            if (val < 0)
                return throw_error(q, head, q->latest_ctx, "representation_error",
                                   "character_code");
...
static bool bif_iso_number_codes_2(query *q)
{
    ...
    if (!is_var(p2) && !any_vars) {
        ...
        if (!is_number(&p->v) || p->error || p->nesting_parens || p->nesting_braces ||
            ...
            SB_free(pr);
            bool ok = throw_error(q, orig_p2, p2_ctx, "syntax_error",
                                  p->error ? p->error_desc : "unexpected_char");
            return ok;
        }
...
static bool do_sub_atom(query *q, cell *p1, cell *p2, pl_ctx p2_ctx, cell *p3, pl_ctx p3_ctx,
                        cell *p4, pl_ctx p4_ctx, cell *p5)
{
    if (!q->retry) {
        q->st.v1 = 0;
    }
...
static bool bif_iso_sub_string_5(query *q)
{
    ...
    bool fixed =
        ((is_integer(p2) ? 1 : 0) + (is_integer(p3) ? 1 : 0) + (is_integer(p4) ? 1 : 0)) >= 2;
...
{
    snprintf(buf, buflen, "%016" PRIx64 "-%04" PRIx64 "-%012" PRIx64 "", u->u1, (u->u2 >> 48),
             (u->u2 & MASK_FINAL));
...
static bool bif_iso_univ_2(query *q)
{
    ...
    if (is_var(p1)) {
        ...
        if (is_var(p22))
            return throw_error(q, p2, p2_ctx, "instantiation_error",
...
static bool do_copy_term(query *q, bool copy_attrs)
{
    ...
    cell *tmp =
        copy_term_to_heap_with_replacement(q, p1, p1_ctx, copy_attrs, p1r, p1r_ctx, p2r, p2r_ctx);
    q->dump_var_num = -1;
    q->dump_var_ctx = -1;
    checked(tmp);
...
static bool bif_sys_current_prolog_flag_2(query *q)
{
    ...
    if (!CMP_STRING_TO_CSTR(q, p1, "double_quotes")) {
        ...
        cell tmp;
        make_atom(&tmp, q->st.m->flags.unknown == UNK_ERROR        ? new_atom(q->pl, "error")
...
static bool bif_iso_set_prolog_flag_2(query *q)
{
    ...
    } else if (!CMP_STRING_TO_CSTR(q, p1, "bounded") || !CMP_STRING_TO_CSTR(q, p1, "max_arity") ||
               !CMP_STRING_TO_CSTR(q, p1, "max_integer") ||
               !CMP_STRING_TO_CSTR(q, p1, "min_integer") || !CMP_STRING_TO_CSTR(q, p1, "version") ||
               !CMP_STRING_TO_CSTR(q, p1, "version_data") ||
               !CMP_STRING_TO_CSTR(q, p1, "version_git") ||
               !CMP_STRING_TO_CSTR(q, p1, "encoding") || !CMP_STRING_TO_CSTR(q, p1, "unix") ||
               !CMP_STRING_TO_CSTR(q, p1, "threads") || !CMP_STRING_TO_CSTR(q, p1, "verbose") ||
               !CMP_STRING_TO_CSTR(q, p1, "integer_rounding_function") ||
               !CMP_STRING_TO_CSTR(q, p1, "dialect") || !CMP_STRING_TO_CSTR(q, p1, "pid")) {
...
static bool bif_help_0(query *q)
{
    ...
    while (sl_next(iter, (void **)&fn)) {
        if (fn->arity)
            fprintf(stdout, "%s/%u: %s(%s)%s%s\n", fn->name, fn->arity, fn->name,
                    fn->help ? fn->help : "no args", fn->iso ? " [ISO]" : "",
                    fn->evaluable ? " [EVALUABLE]" : "");
        else
            fprintf(stdout, "%s/%u: %s%s%s\n", fn->name, fn->arity, fn->name,
                    fn->iso ? " [ISO]" : "", fn->evaluable ? " [EVALUABLE]" : "");
    }
...
static bool bif_help_1(query *q)
{
    ...
    if (!p1->arity) {
        ...
        while (sl_next_key(iter, (void **)&fn)) {
            if (fn->help_alt) {
                if (fn->arity)
                    fprintf(stdout, "%% %s/%u: %s(%s)%s%s\n", fn->name, fn->arity, fn->name,
                            fn->help_alt ? fn->help_alt : "no args", fn->iso ? " [ISO]" : "",
                            fn->evaluable ? " [EVALUABLE]" : "");
            }
    ...
    if (arity)
        fprintf(stdout, "%% %s/%u: %s(%s)%s%s\n%s\n", fn->name, arity, fn->name,
                fn->help ? fn->help : "no args", fn->iso ? " [ISO]" : "",
                fn->evaluable ? " [EVALUABLE]" : "", fn->desc ? fn->desc : "");
    else
        fprintf(stdout, "%% %s/%u: %s%s%s\n%s\n", fn->name, arity, fn->name,
                fn->iso ? " [ISO]" : "", fn->evaluable ? " [EVALUABLE]" : "",
                fn->desc ? fn->desc : "");
...
static bool bif_help_2(query *q)
{
    ...
    if (!strcmp(pr, "swi"))
        snprintf(url, sizeof(url), "http://swi-prolog.org/pldoc/man?predicate=%s/%u", functor,
                 arity);
    else if (!strcmp(pr, "tau"))
        snprintf(url, sizeof(url), "http://tau-prolog.org/documentation/prolog/builtin/%s/%u",
                 functor, arity);
...
static bool bif_module_help_2(query *q)
{
    ...
    if (arity)
        ...
        fprintf(stdout, "%% %s/%u: %s%s%s\n", fn->name, arity, fn->name, fn->iso ? " [ISO]" : "",
                fn->evaluable ? " [EVALUABLE]" : "");
...
static bool do_profile(query *q)
{
    ...
    for (module *m = list_front(&q->pl->modules); m; m = list_next(m)) {
        for (predicate *pr = list_front(&m->predicates); pr; pr = list_next(pr)) {
            for (rule *r = pr->head; r; r = r->next) {
                if (!r->attempted)
                    continue;
...
bool bif_statistics_0(query *q)
{
    fprintf(stdout,
            ...
            q->total_inferences, q->total_matches, q->total_matched, q->hw_frames, q->hw_choices,
            q->hw_trails, q->hw_slots, q->hw_heap_num, q->hw_deref, q->realloc_frames,
            q->realloc_choices, q->realloc_trails, q->realloc_slots, q->st.new_fp, q->cp, q->st.tp,
            q->st.sp, q->st.heap_num, q->total_backtracks, q->total_retries, q->total_tcos,
            q->total_recovs, q->total_no_recovs, (unsigned)q->qcnt[q->st.qnum]);
    return true;
}
...
static bool bif_must_be_4(query *q)
{
    ...
    if (!strcmp(src, "var") && !is_var(p1))
        return throw_error2(q, p1, p1_ctx, "uninstantiation_error", "not_sufficiently_instantiated",
                            p3);
    ...
    if (!strcmp(src, "callable") && !is_callable(p1))
        ...
        if (has_vars(q, p1, p1_ctx))
            return throw_error(q, p1, p1_ctx, "instantiation_error",
                               "not_sufficiently_instantiated");
...
static bool bif_sys_skip_max_list_4(query *q)
{
    ...
    if (is_atomic(p3) && !is_string(p3)) {
        cell tmp;
        make_int(&tmp, 0);
        return unify(q, p1, p1_ctx, &tmp, q->st.cur_ctx) && unify(q, p3, p3_ctx, p4, p4_ctx);
    }
...
char *url_encode(const char *src, int len, char *dstbuf)
{
    ...
    while (len-- > 0) {
        if (*src == ' ') {
            *dst++ = '+';
            src++;
        } else if (!isalnum(*src) && (*src != '-') && (*src != '_') && (*src != '.') &&
                   (*src != '~')) {
...
static void do_template(char *tmpbuf, const char *name, unsigned arity, const char *help,
                        bool function, bool quote)
{
    SB(t);
...
static bool bif_sys_evaluable_property_2(query *q)
{
    ...
    if (CMP_STRING_TO_CSTR(q, p2, "built_in") && CMP_STRING_TO_CSTR(q, p2, "static") &&
        CMP_STRING_TO_CSTR(q, p2, "dynamic") && CMP_STRING_TO_CSTR(q, p2, "foreign"))
...
static bool bif_char_type_2(query *q)
{
    ...
    if (!CMP_STRING_TO_CSTR(q, p2, "alpha"))
        ...
        return iswblank(ch) || iswspace(ch) || (ch == 0x85) || (ch == 0xA0) || (ch == 0x2007) ||
               (ch == 0x202f);
        ...
        return iswspace(ch) || (ch == '\t') || (ch == '\v') || (ch == '\f') || (ch == '\r') ||
               (ch == '\n');
...
static bool bif_string_length_2(query *q)
{
    ...
    if (q->st.m->flags.double_quote_chars && !is_cyclic_term(q, p1, p1_ctx) &&
                     ...
                     const char *type, bool function)
{
    ...
    if (needs_quoting(m, name, strlen(name))) {
        ...
        dst += snprintf(dst, buflen - (dst - tmpbuf), "'$predicate_property'(%s, '%s'",
                        function ? "function" : "predicate", dst2);
        free(dst2);
    } else
        dst += snprintf(dst, buflen - (dst - tmpbuf), "'$predicate_property'(%s, %s",
                        function ? "function" : "predicate", name);
...
static void load_properties(module *m)
{
    ...
    format_property(m, tmpbuf, sizeof(tmpbuf), "!", 0, "choice_construct", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "true", 0, "choice_construct", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "fail", 0, "choice_construct", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), ",", 2, "choice_construct", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), ";", 2, "choice_construct", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "->", 2, "choice_construct", false);
    ...
    format_property(m, tmpbuf, sizeof(tmpbuf), "\\+", 1, "meta_predicate((\\+0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "catch", 3, "meta_predicate(catch(0,?,0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "reset", 3, "meta_predicate(parser_reset(0,?,?))",
                    false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "", 2, "meta_predicate((0,0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), ",", 2, "meta_predicate((0,0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), ";", 2, "meta_predicate((0;0))", false);
    ...
    format_property(m, tmpbuf, sizeof(tmpbuf), "*->", 2, "meta_predicate((0*->0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "if", 3, "meta_predicate(if(0,0,0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "once", 1, "meta_predicate(once(0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "ignore", 1, "meta_predicate(ignore(0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "call", 1, "meta_predicate(call(0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "task", 1, "meta_predicate(task(0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "findall", 3, "meta_predicate(findall(?,0,-))",
                    false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "engine_create", 4,
                    "meta_predicate(engine_create(?,0,?,+))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "|", 2, "meta_predicate(((:)|(+)))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "time", 1, "meta_predicate(time(0))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "call_nth", 2, "meta_predicate(call_nth(0,?))",
                    false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "asserta", 1, "meta_predicate(asserta(:))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "asserta", 2, "meta_predicate(asserta(:,-))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "assertz", 1, "meta_predicate(assertz(:))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "assertz", 2, "meta_predicate(assertz(:,-))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "retract", 1, "meta_predicate(retract(:))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "retract", 2, "meta_predicate(retract(:,?))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "retractall", 1, "meta_predicate(retractall(:))",
                    false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "current_predicate", 1,
                    "meta_predicate(current_predicate(:))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "predicate_property", 2,
                    "meta_predicate(predicate_property(:,?))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "abolish", 1, "meta_predicate(abolish(:))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "clause", 2, "meta_predicate(clause(:,?))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "clause", 3, "meta_predicate(clause(:,?,?))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "bb_b_put", 2, "meta_predicate(bb_b_put(:,?))",
                    false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "bb_put", 2, "meta_predicate(bb_put(:,?))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "bb_get", 2, "meta_predicate(bb_get(:,?))", false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "bb_update", 3, "meta_predicate(bb_update(:,?,?))",
                    false);
    SB_strcat(pr, tmpbuf);
    format_property(m, tmpbuf, sizeof(tmpbuf), "bb_delete", 2, "meta_predicate(bb_delete(:,?))",
                    false);
    SB_strcat(pr, tmpbuf);
    ...
    for (int i = 2; i <= 7; i++) {
        ...
        format_property(m, tmpbuf, sizeof(tmpbuf), "call", i, metabuf, false);
        SB_strcat(pr, tmpbuf);
    }
    ...
    for (int i = 2; i <= 7; i++) {
        ...
        snprintf(dst2, sizeof(metabuf) - (dst2 - metabuf), "))");
        format_property(m, tmpbuf, sizeof(tmpbuf), "task", i, metabuf, false);
        SB_strcat(pr, tmpbuf);
    }
...
static void load_flags(query *q)
{
    ...
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "double_quotes",
               m->flags.double_quote_atom    ? "atom"
               : m->flags.double_quote_chars ? "chars"
               : m->flags.double_quote_codes ? "codes"
                                             : "???");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "char_conversion",
               m->flags.char_conversion ? "on" : "off");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "occurs_check",
               m->flags.occurs_check == OCCURS_CHECK_TRUE    ? "true"
               : m->flags.occurs_check == OCCURS_CHECK_FALSE ? "false"
                                                             : "error");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "character_escapes",
               m->flags.character_escapes ? "true" : "false");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "strict_iso",
               m->flags.strict_iso ? "on" : "off");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "debug", m->flags.debug ? "on" : "off");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "unknown",
               m->flags.unknown == UNK_ERROR        ? "error"
               : m->flags.unknown == UNK_WARNING    ? "warning"
               : m->flags.unknown == UNK_CHANGEABLE ? "changeable"
                                                    : "fail");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "encoding", "'UTF-8'");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "unix", "true");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "threads", "false");
    SB_sprintf(pr, "'$current_prolog_flag'(%s, %s).\n", "global_bb",
               q->pl->global_bb ? "true" : "false");
    ...
    SB_sprintf(pr, "'$current_prolog_flag'(%s, [max_depth(%u),quoted(%s),double_quotes(%s)]).\n",
               ...
               q->pl->def_quoted ? "true" : "false", q->pl->def_double_quotes ? "true" : "false");
...
builtins g_iso_bifs[] = {
    ...
    {"sub_string", 5, bif_iso_sub_string_5,
     "+character_list,?before,?length,?after,?character_list", true, false, BLAH},
    ...
    {"current_predicate", 1, bif_iso_current_predicate_1, "+predicate_indicator", true, false,
     BLAH},
    {"acyclic_term", 1, bif_iso_acyclic_term_1, "+term", true, false, BLAH},
    {"compare", 3, bif_iso_compare_3, "+atom,+term,+term", true, false, BLAH},
    {"unify_with_occurs_check", 2, bif_iso_unify_with_occurs_check_2, "+term,+term", true, false,
     BLAH},
...
builtins g_other_bifs[] = {
    ...
    {"crypto_n_random_bytes", 2, bif_crypto_n_random_bytes_2, "+integer,-codes", false, false,
     BLAH},
    ...
    {"$legacy_current_prolog_flag", 2, bif_sys_current_prolog_flag_2, "+atom,?term", true, false,
     BLAH},
    {"$legacy_predicate_property", 2, bif_sys_predicate_property_2, "+callable,?string", false,
     false, BLAH},
    {"$legacy_evaluable_property", 2, bif_sys_evaluable_property_2, "+callable,?string", false,
     false, BLAH},
    ...
    {"$skip_max_list", 4, bif_sys_skip_max_list_4, "?integer,?integer?,?term,?term", false, false,
     BLAH},
    {"$integer_in_radix", 3, bif_sys_integer_in_radix_3, "+integer,+integer,-string", false, false,
     BLAH},
