                  ...
                  cell *goal)
{
    ...
    if (!strcmp(err_type, "type_error") && ((ptr = strstr(tmpbuf, "_or")) != NULL))
        *ptr = '\0';
    ...
    if (is_var(c) && !q->cycle_error) {
        ...
        // printf("error(%s(%s,(%s)/%u),(%s)/%u).\n", err_type, expected, C_STR(q, c), c->arity,
        // functor, goal->arity);
        tmp = alloc_heap(q, 9);
        checked(tmp);
        pl_idx num_cells = 0;
        ...
        share_cell(c);
        if (is_callable(c)) {
            tmp[num_cells].arity = 0;
            tmp[num_cells].num_cells = 1;
            CLR_OP(tmp + num_cells);
        }
        num_cells++;
        ...
        make_atom(tmp + num_cells++, new_atom(q->pl, functor));
        make_int(tmp + num_cells, !is_string(goal) ? goal->arity : 0);
    } else if (!strcmp(err_type, "permission_error") && is_compound(c) &&
               CMP_STRING_TO_CSTR(q, c, "/") && is_var(FIRST_ARG(c))) {
        // printf("error(%s(%s,(%s)/%u),(%s)/%u).\n", err_type, expected, tmpbuf, c->arity, functor,
        // goal->arity);
        ...
        make_atom(tmp + num_cells++, new_atom(q->pl, functor));
        make_int(tmp + num_cells, !is_string(goal) ? goal->arity : 0);
    } else if (!strcmp(err_type, "permission_error") && (is_builtin || (is_op && c->arity)) &&
               !is_abolish) {
        // printf("error(%s(%s,(%s)/%u),(%s)/%u).\n", err_type, expected, tmpbuf, c->arity, functor,
        // goal->arity);
        tmp = alloc_heap(q, 9 + extra);
        ...
        make_atom(tmp + num_cells++, new_atom(q->pl, functor));
        make_int(tmp + num_cells, !is_string(goal) ? goal->arity : 0);
    } else if (!strcmp(err_type, "representation_error") || !strcmp(err_type, "evaluation_error") ||
               !strcmp(err_type, "syntax_error") || !strcmp(err_type, "resource_error")) {
...
const builtins g_control_bifs[] = {
    ...
    {"call", 8, bif_iso_call_n, ":callable,?term,?term,?term,?term,?term,?term,?term", true, false,
     BLAH},
