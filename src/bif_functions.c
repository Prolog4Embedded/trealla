...
#define ON_OVERFLOW(op, v1, v2)                                                                    \
    ...
    if ((v1) >= INT32_MAX || (v1) <= INT32_MIN || (v2) >= INT32_MAX || (v2) <= INT32_MIN)
#endif
    ...
    if (is_smallint(&p1) && is_smallint(&p2)) {                                                    \
        ...
        if (is_bigint(&p2)) {                                                                      \
            ...
            if (isinf(q->accum.val_float))                                                         \
                return throw_error(q, &q->accum, q->st.cur_ctx, "evaluation_error",                \
                                   "float_overflow");                                              \
        ...
        return throw_error(q, &p1, q->st.cur_ctx, "instantiation_error",                           \
...
bool bif_iso_integer_1(query *q)
{
    ...
    if (q->eval) {
        ...
        if (is_float(&p1) && (p1.val_float < (pl_flt)PL_INT_MAX) &&
...
static bool bif_rdiv_2(query *q)
{
    ...
    if (is_rational(&p1) && is_rational(&p2)) {
        if (mp_int_mul(&p1.val_bigint->irat.num, &p2.val_bigint->irat.den, &q->tmp_irat.num) ==
            MP_MEMORY)
            return throw_error(q, &p1, q->st.cur_ctx, "resource_error", "memory");
        if (mp_int_mul(&p1.val_bigint->irat.den, &p2.val_bigint->irat.num, &q->tmp_irat.den) ==
            MP_MEMORY)
            return throw_error(q, &p1, q->st.cur_ctx, "resource_error", "memory");
    } else if (is_rational(&p1) && is_bigint(&p2)) {
        if (mp_int_mul(&p1.val_bigint->irat.den, &p2.val_bigint->ival, &q->tmp_irat.den) ==
            MP_MEMORY)
            return throw_error(q, &p1, q->st.cur_ctx, "resource_error", "memory");
...
static bool bif_iso_max_2(query *q)
{
    ...
    if (is_smallint(&p1) && is_smallint(&p2)) {
        ...
        if (is_bigint(&p2)) {
            ...
            if (isinf(f1))
                return throw_error(q, &p1, q->st.cur_ctx, "evaluation_error", "float_overflow");
        ...
        if (is_smallint(&p1)) {
            ...
            if (isinf(f2))
                return throw_error(q, &p2, q->st.cur_ctx, "evaluation_error", "float_overflow");
...
static bool bif_log_2(query *q)
{
    ...
    if (is_var(&p1)) {
        ...
        return throw_error(q, &p2, q->st.cur_ctx, "instantiation_error",
                           "not_sufficiently_instantiated");
...
builtins g_evaluable_bifs[] = {
    ...
    {"float_fractional_part", 1, bif_iso_float_fractional_part_1, "+float,-float", true, true,
     BLAH},
