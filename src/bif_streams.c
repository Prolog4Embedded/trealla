...
static void add_stream_properties(query *q, int n)
{
    ...
    *dst = '\0';
    off_t pos =
        !str->is_map && !str->is_engine && !str->is_queue && !str->is_mutex ? ftello(str->fp) : 0;
    bool at_end_of_file = false;

    if (!str->at_end_of_file && (n > 2) && !str->is_engine && !str->is_map && !str->is_queue &&
        !str->is_mutex && !str->p) {
    ...
    while (sl_next(iter, NULL)) {
        ...
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, alias('%s')).\n", n, dst2);
        free(dst2);
    }
    ...
    if (!str->is_engine && !str->is_map && !str->is_queue && !str->is_mutex) {
        ...
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, file_name('%s')).\n", n, dst2);
        free(dst2);
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, file_no(%u)).\n", n, fileno(str->fp));

        dst +=
            snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf), "'$stream_property'(%d, file(%llu)).\n",
        ...
        dst +=
            snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                     "'$stream_property'(%d, line_count(%d)).\n", n, str->p ? str->p->line_num : 1);
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, position(%llu)).\n", n,
                        (unsigned long long)(pos != -1 ? pos : 0));
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, reposition(%s)).\n", n,
                        (n < 3) || str->socket ? "false" : "true");
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, end_of_stream(%s)).\n", n,
                        str->at_end_of_file ? "past"
                        : at_end_of_file    ? "at"
                                            : "not");
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, eof_action(%s)).\n", n,
                        str->eof_action == eof_action_eof_code ? "eof_code"
                        : str->eof_action == eof_action_error  ? "error"
                        : str->eof_action == eof_action_reset  ? "reset"
                                                               : "none");

        if (!str->binary) {
            dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                            "'$stream_property'(%d, bom(%s)).\n", n, str->bom ? "true" : "false");
            dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                            "'$stream_property'(%d, encoding('%s')).\n", n, "UTF-8");
        }

        if (!strcmp(str->mode, "read"))
            dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                            "'$stream_property'(%d, input).\n", n);
        else
            dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                            "'$stream_property'(%d, output).\n", n);

        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, newline(%s)).\n", n, NEWLINE_MODE);
    }

    if (str->is_engine)
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, engine(true)).\n", n);
    else if (str->is_map)
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, skiplist(true)).\n", n);
    else if (str->is_mutex)
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, mutex(true)).\n", n);
    else if (str->is_queue)
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, queue(true)).\n", n);
    else if (str->is_alias)
        dst += snprintf(dst, sizeof(tmpbuf) - strlen(tmpbuf),
                        "'$stream_property'(%d, alias(true)).\n", n);
...
static const char *s_properties = "alias,file_name,mode,encoding,type,line_count,"
...
static bool bif_popen_4(query *q)
{
    ...
    while (is_list(p4)) {
        ...
        if (is_var(c))
            return throw_error(q, c, q->latest_ctx, "instantiation_error",
                               "args_not_sufficiently_instantiated");
        ...
        if (is_var(p4))
            return throw_error(q, p4, p4_ctx, "instantiation_error",
                               "args_not_sufficiently_instantiated");
    }
...
static bool bif_process_create_3(query *q)
{
    ...
    !defined(POSIX_SPAWN_SETSID)
                ...
                return throw_error(q, c, c_ctx, "not available",
            ...
            } else if (!CMP_STRING_TO_CSTR(q, c, "stdin") &&
                       !CMP_STRING_TO_CSTR(q, name, "stream")) {
            ...
            } else if (!CMP_STRING_TO_CSTR(q, c, "stdout") &&
                       !CMP_STRING_TO_CSTR(q, name, "null")) {
                posix_spawn_file_actions_addopen(&file_actions, 1, "/dev/null", O_WRONLY, 0);
            } else if (!CMP_STRING_TO_CSTR(q, c, "stdout") &&
                       !CMP_STRING_TO_CSTR(q, name, "pipe") && is_compound(name) &&
                       (name->arity == 1) && is_var(name + 1)) {
                cell *ns = deref(q, name + 1, name_ctx);
                pl_ctx ns_ctx = q->latest_ctx;
                int n = new_stream(q->pl);
                int fds[2];
            ...
            } else if (!CMP_STRING_TO_CSTR(q, c, "stderr") &&
                       !CMP_STRING_TO_CSTR(q, name, "null")) {
    ...
    pid_t pid;
    int ok = posix_spawnp(&pid, C_STR(q, p1), &file_actions, &attrp, (char *const *)arguments,
...
static bool bif_iso_close_2(query *q)
{
    ...
    while (is_list(p1)) {
        ...
        if (!is_compound(h) || CMP_STRING_TO_CSTR(q, h, "force") ||
...
static bool bif_iso_nl_0(query *q)
{
    ...
    if (str->binary)
        return throw_error(q, q->st.instr, q->st.cur_ctx, "permission_error",
...
static bool bif_sys_read_term_from_chars_4(query *q)
{
    ...
    } else if ((srclen = scan_is_chars_list2(q, p_chars, p_chars_ctx, false, &has_var, &is_partial,
                                             NULL)) > 0) {
        if (!srclen)
            return throw_error(q, p_chars, p_chars_ctx, "type_error", "character");
...
static bool bif_read_term_from_chars_3(query *q)
{
    ...
    if (is_atom(p_chars) && !is_string(p_chars)) {
        ...
        return throw_error(q, p_chars, p_chars_ctx, "type_error", "list");
    } else if ((len = scan_is_chars_list2(q, p_chars, p_chars_ctx, false, &has_var, &is_partial,
                                          NULL)) > 0) {
        if (!len)
            return throw_error(q, p_chars, p_chars_ctx, "type_error", "character");
...
static bool bif_read_file_to_string_3(query *q)
{
    ...
    while (is_list(p3)) {
        ...
        if (is_var(p3))
            return throw_error(q, p3, p3_ctx, "instantiation_error",
                               "args_not_sufficiently_instantiated");
    }
...
static bool bif_access_file_2(query *q)
{
    ...
    if (status && (!CMP_STRING_TO_CSTR(q, p2, "read") || !CMP_STRING_TO_CSTR(q, p2, "exist") ||
                   !CMP_STRING_TO_CSTR(q, p2, "execute") || !CMP_STRING_TO_CSTR(q, p2, "none"))) {
        free(filename);
        return false;
    }
...
static void parse_host(const char *src, char hostname[1024], char path[4096], unsigned *port,
                       int *ssl, int *domain)
{
...
static bool do_parse_parts(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx, bool full)
{
    ...
    if (port)
        SB_sprintf(pr, ":%d", port);
    if (path[0])
        SB_sprintf(pr, "%s", path);
    if (search[0])
        SB_sprintf(pr, "?%s", search);
    if (fragment[0])
        SB_sprintf(pr, "#%s", fragment);
    cell tmp;
...
static bool bif_sys_gsl_vector_write_2(query *q)
{
    ...
    fprintf(str->fp, "#%llu,%ux1,%.1f\n", (unsigned long long)v->size1, (unsigned)v->size1, 0.0);
...
static bool bif_sys_gsl_matrix_write_2(query *q)
{
    ...
    fprintf(str->fp, "#%llu,%ux%u,%.1f\n", (unsigned long long)m->size1 * m->size2,
            (unsigned)m->size1, (unsigned)m->size2, 0.0);
...
builtins g_streams_bifs[] = {
    ...
    {"set_stream_position", 2, bif_iso_set_stream_position_2, "+stream,+integer", true, false,
     BLAH},
    ...
    {"absolute_file_name", 3, bif_absolute_file_name_3, "+source_sink,-atom,+list", false, false,
     BLAH},
    ...
    {"read_term_from_chars", 3, bif_read_term_from_chars_3, "+string,?term,+list", false, false,
     BLAH},
    {"$read_term_from_chars", 4, bif_sys_read_term_from_chars_4, "?term,+list,+string,-string",
     false, false, BLAH},
    {"write_term_to_atom", 3, bif_write_term_to_atom_3, "?atom,?term,+list", false, false, BLAH},
    {"write_canonical_to_atom", 3, bif_write_canonical_to_chars_3, "?atom,?term,+list", false,
     false, BLAH},
    {"write_term_to_chars", 3, bif_write_term_to_chars_3, "?term,+list,?string", false, false,
     BLAH},
    {"write_canonical_to_chars", 3, bif_write_canonical_to_chars_3, "?string,?term,+list", false,
     false, BLAH},
    {"read_line_to_string", 2, bif_read_line_to_string_2, "+stream,-string", false, false, BLAH},
    {"read_file_to_string", 3, bif_read_file_to_string_3, "+source_sink,-string,+options", false,
     false, BLAH},
    ...
    {"$capture_output_to_chars", 1, bif_sys_capture_output_to_chars_1, "-string", false, false,
     BLAH},
    ...
    {"$gsl_matrix_alloc", 3, bif_sys_gsl_matrix_alloc_3, "+stream,-integer,-integer", false, false,
     BLAH},
    {"$gsl_matrix_read", 2, bif_sys_gsl_matrix_read_2, "+integer,+stream", false, false, BLAH},
    {"$gsl_matrix_size", 3, bif_sys_gsl_matrix_size_3, "+integer,-integer,-integer", false, false,
     BLAH},
