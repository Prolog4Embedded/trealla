...
static const op_table g_ops[] = {{":-", OP_XFX, 1200},
                                 {":-", OP_FX, 1200},
                                 {"-->", OP_XFX, 1200},
                                 {"?-", OP_FX, 1200},
                                 {"|", OP_XFY, 1105},
...
predicate *create_predicate(module *m, cell *c, bool *created)
{
    ...
    if (b = get_builtin_term(m, c, &found, &evaluable), !evaluable && found && b->iso) {
...
bool search_goal_expansion(module *m, cell *c)
{
    ...
    for (module *tmp_m = list_front(&m->pl->modules); tmp_m; tmp_m = list_next(tmp_m)) {
        if (m == tmp_m)
            continue;
...
void set_discontiguous_in_db(module *m, const char *name, unsigned arity)
{
    ...
    predicate *pr = find_predicate(m, &tmp);
    if (!pr)
        pr = create_predicate(m, &tmp, NULL);
...
static bool do_use_module(module *curr_m, cell *c, module **mptr)
{
    *mptr = NULL;
    ...
    if (is_compound(p1) && !strcmp(name, "library")) {
        ...
        if (!strcmp(name, "between") || !strcmp(name, "samsort") || !strcmp(name, "terms") ||
            !strcmp(name, "types") || !strcmp(name, "loader") || !strcmp(name, "crypto") ||
            !strcmp(name, "files") || !strcmp(name, "apply") || !strcmp(name, "cont") ||
            !strcmp(name, "os"))
            return true;
...
static bool do_import_predicate(module *curr_m, module *m, predicate *pr, cell *as)
{
    ...
    if (((tmp_pr = find_predicate(curr_m, as)) != NULL) && (curr_m != pr->m) && !pr->m->prebuilt &&
        0) {
        fprintf(stderr, "Error: permission to import failed: %s:%s/%u from %s, see %s\n",
                curr_m->name, C_STR(curr_m, as), as->arity, pr->m->name,
                get_loaded(m, tmp_pr->filename));
        m->error = true;
        return false;
    }
...
bool do_use_module_2(module *curr_m, cell *c)
{
    ...
    while (is_iso_list(p2)) {
        ...
        if (is_interned(head) && (head->arity == 2) &&
            ...
            } else if (is_structure(lhs) && (lhs->arity == 2) && (lhs->val_off == g_slash_s) &&
...
static unsigned search_op_internal(const module *m, const char *name, unsigned *specifier,
                                   bool prefer_unifix)
{
    const op_table *ptr;
...
static unsigned match_op_internal(const module *m, const char *name, unsigned *specifier,
                                  unsigned arity)
{
    const op_table *ptr;
    sliter *iter = sl_find_key(m->defops, name);
...
{
    if (pr->head && !pr->is_multifile && !pr->is_dynamic && (C_STR(m, &pr->key)[0] != '$')) {
...
static void process_cell(module *m, clause *cl, cell *c, predicate *parent, int last_was_colon,
                         bool is_directive)
{
    cell *body = cl->cells;
    unsigned specifier;

    if ((c->arity == 2) && !GET_OP(c) && (c->val_off != g_braces_s) &&
...
module *load_text(module *m, const char *src, const char *filename)
{
    ...
    if (!p->error) {
        ...
        if (p->m->run_init) {
            ...
            SB(src);
            SB_sprintf(src,
                       "forall(%s:retract(('$directive'(initialization(__G_)))), (once(__G_); "
                       "format('Error: ~w~n', [__G_])))",
                       p->m->name);
