...
static bool bif_iso_retractall_1(query *q)
{
    ...
    if (!pr) {
        ...
        if (get_builtin_term(q->st.m, head, &found, NULL), found)
            return throw_error(q, head, q->latest_ctx, "permission_error",
...
static bool bif_iso_abolish_1(query *q)
{
    ...
    if (get_builtin(q->pl, C_STR(q, p1_name), C_STRLEN(q, p1_name), get_smallint(p1_arity), &found,
                    NULL),
        found) {
        prolog_unlock(q->pl);
...
static bool bif_iso_asserta_1(query *q)
{
    ...
    if (is_var(head))
        return throw_error(q, head, q->st.cur_ctx, "instantiation_error",
                           "args_not_sufficiently_instantiated");
    ...
    if (get_builtin_term(q->st.m, head, &found, NULL), found) {
        if (!GET_OP(head)) {
            return throw_error(q, head, q->st.cur_ctx, "permission_error",
                               "modify,static_procedure");
        }
    }
...
void save_db(FILE *fp, query *q, int logging)
{
    ...
    for (predicate *pr = list_front(&q->st.m->predicates); pr; pr = list_next(pr)) {
        if (pr->is_builtin)
            continue;
...
static bool bif_abolish_2(query *q)
{
    ...
    if (!force) {
        ...
        if (get_builtin(q->pl, C_STR(q, p1_name), C_STRLEN(q, p1_name), get_smallint(p1_arity),
                        &found, NULL),
            found) {
            return throw_error(q, p1, p1_ctx, "permission_error", "modify,static_procedure");
        }
...
static bool do_dump_term(query *q, cell *p1x, pl_ctx p1x_ctx, cell *p1, pl_ctx p1_ctx, bool deref,
                         int depth)
{
    if (!depth) {
    ...
    for (unsigned i = 0; i < p1->num_cells; i++, tmp++) {
        ...
        if (is_var(tmp))
            printf(", global=%d, void=%d, local=%d, temp=%d, anon=%d", is_global(tmp) ? 1 : 0,
                   is_void(tmp) ? 1 : 0, is_local(tmp) ? 1 : 0, is_temporary(tmp) ? 1 : 0,
                   is_anon(tmp) ? 1 : 0);
